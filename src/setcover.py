class SetCover:
    ...
    def find_smallest_team_greedy_method(self, req_skills, people):
        ...
        self.dfs_greedy_method(
            0, req_skills_index_map, people_skills_map, skill_people_map, 0, 0, 0
        )
        return self.convert_to_indices(self.rv, len(people))

    def dfs_greedy_method(
        self,
        req_skill,
        req_skills_index_map,
        people_skills_map,
        skill_people_map,
        skillset,
        peopleset,
    ...
    def find_smallest_team_dp(self, req_skills, people):
            ...
            for j in range(1, pow(2, M)):
                if skills_with_person & j == 0:
                    ...
                    if remaining_skills == 0:
                        ...
                        if key_in_dict(i, j) and (
                            (dp[i][remaining_skills][1] + 1) <= dp[i - 1][j][1]
                        ):
        ...
        return self.convert_to_indices(
            dp[row_length - 1][col_length - 1][0], row_length
        )
