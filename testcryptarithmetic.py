...
def test2():
    ...
    found = False
    for r in result:
        if r['D'] == 7 and r['E'] == 5 and r['Y'] == 2 and r['N'] == 6 and r['R']== 8 and r['O'] == 0 and r['S'] == 9 and r['M'] == 1:
            found = True
 
    assert found

def testisvalid6():
    letters = OrderedDict([('S', 0), ('E', 1), ('N', 2), ('D', None), ('M', None), ('O', None), ('R', None), ('Y', None)])
    
    wordsArr = [['#', 'S', 'E', 'N', 'D'], ['#', 'M', 'O', 'R', 'E'], ['M', 'O', 'N', 'E', 'Y']]
    crypt = Cryptarithmetic([])
    crypt.normalizeWords(wordsArr)
    letters = {}
